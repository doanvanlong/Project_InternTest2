...
urlElement('http://'.$config_url.'/bao-gia.html','1.0'); 
urlElement('http://'.$config_url.'/dich-vu.html','1.0'); 
...
///////////dich-vu//////////////////////
	$d->reset();
    $sql = "select id,ten$lang as ten,tenkhongdau from #_news where hienthi=1 and type='dich-vu' order by stt asc, id desc";
    $d->query($sql);
    $blog = $d->result_array(); 

foreach($blog as $v){
	urlElement('http://'.$config_url.'/dich-vu/'.$v['tenkhongdau'].'.html','0.8');
}
